...
void printdev(libusb_device *dev){
 ...
 uint8_t* serialNum = new uint8_t[serNumBuf];
 uint8_t* product = new uint8_t[serNumBuf];
 ...
 try {
    ...
    if (handle != nullptr && libusb_get_string_descriptor_ascii(handle, desc.iSerialNumber, serialNum, serNumBuf - 2) >= 0) {
        serialNum[serNumBuf - 1] = '\0';
        cout << "SerialNum: " << serialNum;
    }

    cout << "             ";

    if (handle != nullptr && libusb_get_string_descriptor_ascii(handle, desc.iProduct, product, serNumBuf - 2) >= 0) {
        product[serNumBuf - 1] = '\0';
